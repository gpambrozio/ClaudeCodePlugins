...
- Added `SessionEnd` hook (`teardown-sandbox.py`) to clean up sandbox directories when the session ends
- Simplified `setup-sandbox.sh` — creates per-session directories keyed by session ID; pointer file removed
