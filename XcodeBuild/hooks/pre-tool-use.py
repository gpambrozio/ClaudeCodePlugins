...
def main():
    try:
        ...
        # Check for xcodebuild or swift build/test as a complete word (word boundary)
        if tool_name == "Bash" and re.search(r'^[/a-zA-Z]?(xcodebuild|swift build|swift test)\s', command):
            # Remind to use the xcodebuild skill
            response = {
                "hookSpecificOutput": {
                    "hookEventName": "PreToolUse",
                    "permissionDecision": "allow",
                    "permissionDecisionReason": "Make sure to use the xcodebuild skill to compile with xcodebuild or swift."
                }
            }
